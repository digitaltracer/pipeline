...
public final class PlatformDetectionService {
    public static let shared = PlatformDetectionService()
    ...
    public func detectPlatform(from urlString: String?) -> Platform {
    ...
    public func detectPlatform(from url: URL?) -> Platform {
        return detectPlatform(from: url?.absoluteString)
    }

    /// Extract the job ID from common job posting URLs
    public func extractJobID(from urlString: String?) -> String? {
    ...
    public func isJobPostingURL(_ urlString: String?) -> Bool {
